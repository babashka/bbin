...
**1. Install `bbin` CLI:**
```shell
mkdir -p ~/.local/bin && curl -o- -L https://raw.githubusercontent.com/babashka/bbin/v0.2.0-SNAPSHOT/bbin > ~/.local/bin/bbin && chmod +x ~/.local/bin/bbin
```
...
Invoke-WebRequest -Uri https://raw.githubusercontent.com/babashka/bbin/v0.2.0-SNAPSHOT/bbin -OutFile $Env:HOMEDRIVE$Env:HOMEPATH\.local\bin\bbin
Invoke-WebRequest -Uri https://raw.githubusercontent.com/babashka/bbin/v0.2.0-SNAPSHOT/bbin.bat -OutFile $Env:HOMEDRIVE$Env:HOMEPATH\.local\bin\bbin.bat
```
