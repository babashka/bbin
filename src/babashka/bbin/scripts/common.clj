...
(defn dir-config-strategy [script-root cli-config]
  (let [bb-edn (read-bb-edn script-root)
        author-config (bb-opts->config (:bbin/bb-opts bb-edn))
        has-deps? (fs/exists? (fs/file script-root "deps.edn"))
        has-bb? (fs/exists? (fs/file script-root "bb.edn"))]
    (cond
      cli-config
      {:strategy :explicit-config
       :config (resolve-config script-root cli-config)}

      author-config
      {:strategy :explicit-config
       :config (resolve-config script-root author-config)}
...
(defn dir-template [strategy {:keys [git? tool-mode? lib?]}]
  (case (:strategy strategy)
    :explicit-config
    (if tool-mode? config-tool-template-str config-dir-template-str)

    :no-config
    (if tool-mode?
      (throw (ex-info "Tool mode requires a bb.edn or deps.edn in the directory." {}))
      no-config-dir-template-str)

    :generated
    (cond
      (and tool-mode? lib?) deps-tool-template-str
      tool-mode? local-dir-tool-template-str
      git? git-dir-template-str
      :else local-dir-template-str)))
