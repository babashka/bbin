    ...
    (let [local-root (str (fs/file tu/test-dir "both-config"))]
      ...
      (testing "CLI --bb-opts can select bb.edn"
        ...
        (is (str/includes? (tu/run-bin-script "both-cli-config")
                           "bb-config-ok"))
        (let [contents (bin-contents "both-cli-config")
              bb-edn (str (fs/canonicalize (fs/file local-root "bb.edn")
                                           {:nofollow-links true}))
              meta (scripts/parse-script contents)]
          (is (str/includes? contents "(def script-bb-opts [\"--config\""))
          (is (str/includes? contents "(if (seq script-bb-opts)"))
          (is (= ["--config" bb-edn] (:bbin/bb-opts meta)))
          (is (not (contains? meta :bbin/config)))))

      (testing "CLI --bb-opts rejects non-config opts"
        (is (thrown-with-msg?
             ExceptionInfo
             #"Only --config is supported by --bb-opts"
             (tu/run-install {:script/lib local-root
                              :main-opts "[\"-m\" \"both-script\"]"
                              :bb-opts "[\"--classpath\" \"src\"]"
                              :as "both-invalid-bb-opts"}))))
